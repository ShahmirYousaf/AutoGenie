...
const AddNewCar = () => {
        ...
        if(processImageResult?.success) {
           ...
           // Update form with AI results
            ...
            toast.success("Successfully extracted car details", {
                description: `Deetected ${carDetails.year} ${carDetails.make} ${carDetails.model}
                with ${Math.round(carDetails.confidence * 100)}% confidence`
            })

            setActiveTab("manual");
        }
     }, [processImageResult, setValue, uploadAiImage])
       ...
       validFiles.forEach((file) => {
        ...
        reader.onload = (e) => {
            ...
            if(newImages.length === validFiles.length)
            {
                ...
                setImageError("");
                toast.success(`Sucessfully uploaded ${validFiles.length}`);
            }
        };
  ...
  return (
    <div>
        <Tabs defaultValue= "ai" className="mt-8" value={activeTab} onValueChange={setActiveTab}>
                    ...
                    <CardContent>
                        ...
                        className='space-y-6'>
                                ...
                                <div className='space-y-2'>
                                    ...
                                    <Select  onValueChange={value => setValue("fuelType", value)}
                                        ...
                                        <SelectContent>
                                            ...
                                            {fuelTypes.map((fType)=> {
                                                return  <SelectItem key={fType} value={fType} > {fType} </SelectItem>
                                            })}
                                    ...
                                    <Select onValueChange={value => setValue("bodyType", value)}
                                        ...
                                        <SelectContent>
                                            ...
                                            {bodyTypes.map((bodyType)=> {
                                                return  <SelectItem key={bodyType} value={bodyType} > {bodyType} </SelectItem>
                                            })}
                            ...
                            <Button type="submit" className="w-full md:w-auto" disabled={addCarLoading}>
                             {addCarLoading ? ( <>
                             <Loader2 className='mr-2 h-4 w4 animate-spin'/> Adding Car...
                             </> )
                             : ("Add Car" )}
                            </Button>
                        </form>

...
const Sidebar = () => {
  ...
  return (
    <>
        <div className='hidden md:flex h-full flex-col overflow-y-auto bg-white shadow-sm border-r'>  {/* For WEB UI */}
            {routes.map((route) => {
                return (
                ...
                className={cn(
                    ...
                    : ""
                )}>   {/* Helps us render tailwinds classes conditionally */}
        ...
        <div className='md:hidden fixed bottom-0 left-0 right-0 z-50 bg-white border-t flex justify-around items-center h-16'> {/* For Mobile UI */}
            {routes.map((route) => {
                    return (
                    <Link key={route.href} href={route.href}
                    className={cn(
